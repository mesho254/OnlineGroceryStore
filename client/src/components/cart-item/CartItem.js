    ...
    return (
        <div className={styles['wrapper']}>
            ...
            <div className={styles['price']}>
                <div className={styles['total-price']}>{(parseFloat(product.price) * parseFloat(product.quantity || '1')).toFixed(2)} KES</div>
                {parseInt(product.quantity) > 1 && <div
                    className={styles['unit-price']}>{product.price} KES/{product.weight}{product.measurement}</div>}
            </div>
        </div>
    );
}
